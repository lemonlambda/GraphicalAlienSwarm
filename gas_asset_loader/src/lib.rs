...
pub struct GASImageLoaderPlugin;

impl Plugin for GASImageLoaderPlugin {
    fn build(&self, app: &mut App) {
        app.add_systems(Startup, add_asset_loader);
    }
}
...
impl std::fmt::Display for GASFileTextureError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::result::Result<(), std::fmt::Error> {
        write!(
            f,
            "Error reading image file {}: {}, this is an error in `gas_asset_loader`.",
